...
const changeAvatarImage = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    { new: true }
  ).select("-password -_id -watchHistory -fullname -email -coverImage  -refreshToken -createdAt -updatedAt -username");

  return res
    .status(200)
    .json(new ApiResponse(200, user, "Avatar image upadated successfully"));
});
...
const changeCoverImage = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    { new: true }
  ).select("-password -_id -watchHistory -fullname -email -avatar -refreshToken -createdAt -updatedAt -username");
